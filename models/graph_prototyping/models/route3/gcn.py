...
class ProtoGCNBlock(nn.Module):
    def __init__(self, in_channel=1024, out_channel=2048):
        super(ProtoGCNBlock, self).__init__()
        self.linear = nn.Linear(in_channel, out_channel)
        self.gc1 = GraphConvolution(in_channel, 1024)
        self.gc2 = GraphConvolution(1024, out_channel)
        self.relu = nn.LeakyReLU(0.2)

    def forward(self, proto_features, proto_adj):
        x = self.gc1(proto_features, proto_adj)
        x = self.relu(x)
        x = self.gc2(x, proto_adj)
