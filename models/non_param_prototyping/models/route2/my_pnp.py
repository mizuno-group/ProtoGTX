# -*- coding: utf-8 -*-
"""
Created on 2025-08-01 (Fri) 16:03:13

Non-parametric prototyping for WSI classification.
...
class PCA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, attn_maps=None):
...
class WSI_PNP(nn.Module):
    def __init__(self, *,
        ...
        self.fg_extractor = PCA(bg_class=n_classes, compare_fn="le", threshold=0.5)
    ...
    def forward(self, x: torch.Tensor, *, use_gumbel: bool = False):
        ...
        raw_patch_tokens = x['img']
        labels = x['label']

        assert (not self.training) or (labels is not None)

        patch_tokens = F.normalize(raw_patch_tokens, p=2, dim=-1)
        ...
        if labels is not None:
            ...
            pseudo_patch_labels = self.fg_extractor(raw_patch_tokens.detach(), labels)
...
class PNPCriterion(nn.Module):
    ...
    def forward(self, outputs: dict[str, torch.Tensor], batch: tuple[torch.Tensor, ...]):
        ...
        if self.l_ppd_coef != 0:
            ...
            loss_dict["l_ppd_unadjusted"] = l_ppd
